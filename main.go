	...
	for _, a := range runningAllocs {
		...
		go func(a *api.AllocationListStub) {
			...
			for taskName, taskStats := range stats.Tasks {
				labels := []string{*alloc.Job.Name, alloc.TaskGroup, alloc.Name, taskName, *alloc.Job.Region, node.Datacenter, node.Name}
				ch <- prometheus.MustNewConstMetric(
					taskCPUPercent, prometheus.GaugeValue, taskStats.ResourceUsage.CpuStats.Percent, labels...,
				)
				ch <- prometheus.MustNewConstMetric(
					taskCPUTotalTicks, prometheus.GaugeValue, taskStats.ResourceUsage.CpuStats.TotalTicks, labels...,
				)
				ch <- prometheus.MustNewConstMetric(
					taskMemoryRssBytes, prometheus.GaugeValue, float64(taskStats.ResourceUsage.MemoryStats.RSS), labels...,
				)
			}
			labels := []string{*alloc.Job.Name, alloc.TaskGroup, alloc.Name, *alloc.Job.Region, node.Datacenter, node.Name}
			ch <- prometheus.MustNewConstMetric(
				allocationCPU, prometheus.GaugeValue, stats.ResourceUsage.CpuStats.Percent, labels...,
			)
			ch <- prometheus.MustNewConstMetric(
				allocationCPUThrottled, prometheus.GaugeValue, float64(stats.ResourceUsage.CpuStats.ThrottledTime), labels...,
			)
			ch <- prometheus.MustNewConstMetric(
				allocationMemory, prometheus.GaugeValue, float64(stats.ResourceUsage.MemoryStats.RSS), labels...,
			)
			ch <- prometheus.MustNewConstMetric(
				allocationMemoryLimit, prometheus.GaugeValue, float64(*alloc.Resources.MemoryMB), labels...,
			)
		}(a)
	}
